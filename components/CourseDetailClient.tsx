...
const levelBg: Record<string, string> = {
  Beginner: "rgba(52,211,153,0.12)",
  Intermediate: "rgba(251,191,36,0.12)",
  Advanced: "rgba(248,113,113,0.12)",
};
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      ...
      <div className="r-hero" style={{
        ...
        {/* Decorative blobs */}
        <div style={{ position: "absolute", top: -60, right: -60, width: 240, height: 240, borderRadius: "50%", background: "rgba(109,92,247,0.12)", pointerEvents: "none" }} />
        <div style={{ position: "absolute", bottom: -40, left: "30%", width: 160, height: 160, borderRadius: "50%", background: "rgba(52,211,153,0.08)", pointerEvents: "none" }} />

        <div style={{ position: "relative", maxWidth: 900 }}>
          {/* Breadcrumb */}
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 20, fontSize: 13, color: "var(--subtle)" }}>
            <Link href="/courses" style={{ color: "var(--muted)" }}>Browse Courses</Link>
            <svg viewBox="0 0 24 24" width={12} height={12} fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
              <path d="m9 18 6-6-6-6" />
            </svg>
            <span style={{ color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 260 }}>
              {course.title}
            </span>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 12, color: "var(--subtle)" }}>{total} lessons</span>
            <span style={{ fontSize: 12, color: "var(--subtle)" }}>·</span>
            <span style={{ fontSize: 12, color: "var(--subtle)" }}>Self-paced</span>
            <span style={{ fontSize: 12, color: "var(--subtle)" }}>·</span>
            <span style={{ fontSize: 12, color: "var(--subtle)" }}>Free</span>
          </div>
          ...
          <p style={{ fontSize: 15, color: "var(--muted)", lineHeight: 1.8, maxWidth: 620 }}>
            {course.description}
          </p>
        </div>
      </div>
          ...
          <div style={{
            ...
            {pct < 100 && nextLesson ? (
              <Link href={`/lesson/${nextLesson.slug}`} style={{
                ...
                boxShadow: "0 2px 16px var(--accent-glow)", whiteSpace: "nowrap", flexShrink: 0,
              }}>
                {completedCount === 0 ? "Start Course →" : "Continue →"}
              </Link>
            ) : pct === 100 ? (
              <span style={{
                padding: "10px 22px", background: "rgba(52,211,153,0.1)",
                color: "var(--green)", borderRadius: 10, fontSize: 13, fontWeight: 700,
        ...
        <div className="r-panel-grid" style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 272px", gap: 24, alignItems: "start" }}>
              ...
              {course.lessonList.map((lesson, i) => {
                ...
                return (
                  <div key={lesson.slug} style={{
                    ...
                    padding: "14px 18px",
                    background: done ? "rgba(52,211,153,0.04)" : "var(--card)",
                    ...
                    <div style={{
                      ...
                      background: done ? "rgba(52,211,153,0.1)" : unlocked ? "var(--accent-bg)" : "var(--surface)",
                      color: done ? "var(--green)" : unlocked ? "var(--accent-hover)" : "var(--subtle)",
                      border: `1px solid ${done ? "rgba(52,211,153,0.2)" : unlocked ? "var(--accent-border)" : "var(--border)"}`,
                    }}>
                      {done ? (
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: 12, color: "var(--subtle)" }}>
                        {done ? "Completed" : unlocked ? "Ready to start" : !isEnrolled ? "Enroll to unlock" : "Complete previous lesson first"}
                      </p>
                    </div>
                    ...
                    {unlocked ? (
                      <Link href={`/lesson/${lesson.slug}`} style={{
                        ...
                        color: done ? "var(--subtle)" : "#fff",
                        padding: "6px 14px", borderRadius: 8, flexShrink: 0,
                        background: done ? "var(--surface)" : "var(--accent)",
                        border: done ? "1px solid var(--border)" : "none",
                        boxShadow: done ? "none" : "0 2px 8px var(--accent-glow)",
                        transition: "all 0.15s",
                      }}>
                        {done ? "Review" : "Start"}
                      </Link>
          ...
          <div style={{ position: "sticky", top: 24, display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden" }}>
              ...
              <div style={{
                height: 100,
                background: "linear-gradient(135deg, rgba(109,92,247,0.35) 0%, rgba(167,139,250,0.25) 100%)",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 40,
              }}>
                📘
                ...
                {loading ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    <div style={{
                      ...
                      borderRadius: 10, fontSize: 13, color: "var(--green)", fontWeight: 600,
                    }}>
                      ✓ You are enrolled
                    </div>
                    {pct < 100 && nextLesson && (
                    ...
                    {pct === 100 && (
                      <div style={{
                        ...
                        background: "rgba(52,211,153,0.08)",
                        borderRadius: 10, fontSize: 14, fontWeight: 700, color: "var(--green)",
                      }}>
                        Course Complete 🎉
                      </div>
                    )}
                  ...
                  <div>
                    ...
                    {error && (
                      <p style={{ fontSize: 12, color: "var(--red)", marginTop: 10, textAlign: "center", lineHeight: 1.5 }}>
                        {error}
                      </p>
                    )}
                  </div>
            ...
            <Link href="/courses" style={{
              ...
              fontSize: 13, fontWeight: 500, color: "var(--muted)", textAlign: "center",
            }}>
              ← Back to all courses
            </Link>
          </div>
